...
class RoleButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.limit_to_one:
            roles_to_remove = [
                interaction.guild.get_role(r) for r in self.all_roles
        ...
        try:
            ...
            await interaction.response.send_message("I don't have permissions to manage that role.", ephemeral=True)
...
class ReactionRoles(commands.Cog, name="Reaction Roles"):
    ...
    async def cog_load(self):
        ...
        for message_id_str, data in views.items():
            try:
                self.bot.add_view(PersistentRoleView(data=data), message_id=int(message_id_str))
    ...
    async def new(self, ctx: commands.Context):
        ...
        if not await ctx.prompt("Welcome to the Reaction Light creation program. Proceed?", author_id=ctx.author.id):
            return
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    error_messages.append(await ctx.send("Mention a role after the reaction."))
                    continue
                ...
                try:
                    ...
                    error_messages.append(await ctx.send("Invalid or inaccessible emoji!"))
                    continue
        ...
        sent_limit_message = await ctx.send("Limit to one role per user? React 🔒 for yes, 🔓 for no.")
        ...
        try:
            payload = await self.bot.wait_for("raw_reaction_add", timeout=120, check=reaction_check)
        ...
        try:
            ...
            emb = Embed()
            if len(vals) > 1 and vals[1].lower() != "none": emb.title = vals[1]
            if len(vals) > 2 and vals[2].lower() != "none": emb.description = vals[2]
            ...
            if content or emb:
                ...
                try:
                    ...
                    error_messages.append(await ctx.send("I don't have permission to send to that channel."))
        except asyncio.TimeoutError:
            await ctx.author.send("Timeout.")
        finally:
            await self.delete_messages(user_messages + error_messages)
    ...
    @has_permissions(manage_roles=True)
    async def delete_reaction_roles(self, ctx: commands.Context, reaction_roles_message: commands.MessageConverter):
        """Deletes the reaction roles setup"""
        views = await self.config.global_().get_attr("views", {})
        msg_id_str = str(reaction_roles_message.id)
        
        if msg_id_str not in views:
            return await ctx.send("That message does not have any reaction role associated with it", delete_after=LinksAndVars.delete_message.value)
