...
class Moderation(commands.Cog):
    ...
    async def kick(
        self, ctx, member: Optional[Union[discord.Member, MemberID]], *, reason=None
    ):
    ...
    @commands.has_guild_permissions(ban_members=True)
    async def banlist(
        self, ctx, *, member: Optional[Union[str, MemberID, discord.Member]]
    ):
    ...
    async def warn(
        self, ctx, member: Union[MemberID, discord.Member], *, reason: str = None
    ):
        """Warn a user"""
        member = ctx.get_user(member)
        if not await ctx.prompt(
