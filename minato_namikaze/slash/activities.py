...
class Activities(discord.SlashCommand):
    ...
    async def activities_autocomplete(self, response: discord.AutocompleteResponse) -> typing.AsyncIterator[str]:
        for activity in activities:
            if response.value.lower() in activity.lower():
                yield activity
    
    voice_channel: typing.Optional[discord.VoiceChannel] = discord.application_command_option(description='A voice channel in the selected activity will start')
    
    @voice_channel.autocomplete
    async def voice_channel_autocomplete(self, response: discord.AutocompleteResponse) -> typing.AsyncIterator[discord.VoiceChannel]:
