...
from ...lib import Embed, database_category_name, database_channel_name, ChannelAndMessageId, is_mod
...
class ServerSetup(commands.Cog, name="Server Setup"):
    ...
    async def add_and_check_data(self, dict_to_add: dict, ctx: commands.Context) -> None:
        database = await self.db.new(database_category_name, database_channel_name)
    ...
    async def support(self, ctx, textchannel: commands.TextChannelConverter, support_required_role: commands.RoleConverter):
        '''
        ...
        '''
        if not await ctx.prompt(f'Do you really want to **create a suppoprt system** for **{ctx.guild.name}** in {textchannel.mention}?'):
            return
    ...
    async def badlinks(self, ctx, option: typing.Literal[True, False, 'yes', 'no', 'on', 'off'], action: typing.Optional[typing.Literal['ban', 'mute', 'timeout', 'kick', 'log']] = 'log', logging_channel: typing.Optional[commands.TextChannelConverter] = None):
        '''
