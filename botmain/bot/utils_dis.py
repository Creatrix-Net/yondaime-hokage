...
class VotingMenu(menus.Menu):
    ...
    async def on_check_mark(self, payload):
        e1 = discord.Embed(title="Thanks!", description=f"Thanks {self.ctx.author.mention}! Here's the [link!]({self.bot.topgg})")
...
class WhoMenu(menus.Menu):
    ...
    async def on_add(self, payload):
        e1 = discord.Embed(title="Well, Heres The Policy :)", description=f"Well well well, Nothing is stored! Really nothing is stored! All is based on internal cache provided by Discord!", color=discord.Colour.from_hsv(random.random(), 1, 1))
        await self.message.edit(content="", embed=e1)
        

    @menus.button('\N{NEGATIVE SQUARED CROSS MARK}')
    async def on_stop(self, payload):
        e2 = discord.Embed(title="Hey!", description=f"Hi, I'm {self.bot.user}, I am developed by {self.bot.owner}, Who is a great fan of me i.e. {self.bot.user} aka Yondaime Hokage!", color=discord.Colour.from_hsv(random.random(), 1, 1))
