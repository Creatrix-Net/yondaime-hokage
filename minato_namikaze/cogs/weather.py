...
class Weather(commands.Cog):
    ...
    @commands.bot_has_permissions(embed_links=True)
    async def weather_by_zip(self, ctx: commands.Context, *,
                             zipcode: str) -> None:
        """
        Display weather in a given location
    ...
    @commands.bot_has_permissions(embed_links=True)
    async def weather_by_cityid(self, ctx: commands.Context, *,
                                cityid: int) -> None:
        """
        Display weather in a given location
    ...
    @commands.bot_has_permissions(embed_links=True)
    async def weather_by_coordinates(self, ctx: commands.Context, lat: float,
                                     lon: float) -> None:
        """
        Display weather in a given location
    ...
    async def get_weather(
        ...
        *,
        ...
        windspeed = str(
            data["wind"]["speed"]) + " " + self.unit[units]["speed"]
        if units == "kelvin":
        ...
        sunrise = datetime.datetime.utcfromtimestamp(
            data["sys"]["sunrise"] + data["timezone"]).strftime("%H:%M")
        ...
        if city and country:
            ...
            embed.add_field(name="\N{EARTH GLOBE AMERICAS} **Location**",
                            value="*Unavailable*")
        embed.add_field(name="\N{STRAIGHT RULER} **Lat,Long**",
                        value="{0}, {1}".format(lat, lon))
        ...
        embed.add_field(
            ...
            value="{0:.2f}{1} to {2:.2f}{3}".format(mintemp,
                                                    self.unit[units]["temp"],
                                                    maxtemp,
                                                    self.unit[units]["temp"]),
        )
        embed.add_field(name="\N{SUNRISE OVER MOUNTAINS} **Sunrise**",
                        value=sunrise)
        embed.add_field(name="\N{SUNSET OVER BUILDINGS} **Sunset**",
                        value=sunset)
