...
def upgrade() -> None:
    ...
    sa.Column('custom_id', sa.ARRAY(sa.String(length=250), as_tuple=True, zero_indexes=True), nullable=False),
    sa.PrimaryKeyConstraint('message_id')
    )
    op.create_index(op.f('ix_reaction_roles_channel_id'), 'reaction_roles', ['channel_id'], unique=False)
    op.create_index(op.f('ix_reaction_roles_custom_id'), 'reaction_roles', ['custom_id'], unique=False)
    op.create_index(op.f('ix_reaction_roles_limit_to_one'), 'reaction_roles', ['limit_to_one'], unique=False)
    op.create_index(op.f('ix_reaction_roles_message_id'), 'reaction_roles', ['message_id'], unique=True)
    op.create_index(op.f('ix_reaction_roles_server_id'), 'reaction_roles', ['server_id'], unique=False)
    ...
    op.create_index(op.f('ix_reminders_expires'), 'reminders', ['expires'], unique=False)
    ...
    sa.ForeignKeyConstraint(['premium_applier_user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
