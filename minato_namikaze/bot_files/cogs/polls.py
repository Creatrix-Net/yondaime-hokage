...
class QuickPoll(commands.Cog):
    ...
    async def tally(self, ctx, id):
        ...
        try:
            ...
            await ctx.send(
                embed=ErrorEmbed(description=f"No polls with this {id} found!")
            )
        ...
        embed_result = Embed(
            ...
            description="\n".join(
                [
                    "{} **{}**: {}\n".format(
                        self.reactions[i], opt_dict[key], tally[key]
                    )
    ...
    async def poll_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(
                embed=ErrorEmbed(description="Missing the question."), delete_after=5
            )
