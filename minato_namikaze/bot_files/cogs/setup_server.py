    ...
    async def _setup(self, ctx):
        ...
        if (feed_channel and support_channel and ban_channel and unban_channel
            ...
            if not feed_channel:
                ...
                await ctx.send(
                    f"The channel for logging of feedback is already there {feed_channel.mention}",
                    delete_after=5,
                )
                await feed_channel.send(file=self.pin_img(), embed=embed)
            ...
            if not support_channel:
                ...
                await ctx.send(
                    f"The channel for support is already there {support_channel.mention}",
                    delete_after=5,
                )
                await support_channel.send(file=self.pin_img(), embed=embed)
            ...
            if not ban_channel:
                ...
                await ban_channel.send(file=self.pin_img(), embed=embed)
                await ctx.send(
                    f"Channel for **logging bans already there**! {ban_channel.mention}",
                    delete_after=5,
                )
            ...
            if not warns_channel:
                ...
                await warns_channel.send(file=self.pin_img(), embed=embed)
                await ctx.send(
                    f"Channel for **logging warns already there**! {warns_channel.mention}",
                    delete_after=5,
                )

            # Setup Finish
            await botask.send("**Deleting this setup channel in**")
            gb = await botask.send(commonwaitingtime)
            for i in range(1, commonwaitingtime):
                await gb.edit(content=commonwaitingtime - i)
                await asyncio.sleep(1)
            await botask.delete()
