...
class Unban(menus.Menu):
    ...
    async def send_initial_message(self, ctx, channel):
        embed = Embed(title=f"Want to log unbans for the **{ctx.guild.name}** ?")
    ...
    async def on_add(self, payload):
        bingo = discord.utils.get(self.ctx.guild.categories, name="Bingo Book") if discord.utils.get(self.ctx.guild.categories, name="Bingo Book") else False
        if not bingo:
