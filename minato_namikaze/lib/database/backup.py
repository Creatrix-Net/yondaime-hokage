...
class BackupDatabse:
    ...
    async def apply_backup(self, code:int) -> Optional[Union[str, bool]]:
            ...
            try:
                ...
                try:
                    if not roles[j]['position'] <= 0:
                        await role_in_server.edit(position=roles[j]['position'], reason = self.reason(code, self.ctx.author))
                ...
                try:
                    if not roles[j]['position'] <= 0:
                        await created_role.edit(position=roles[j]['position'], reason = self.reason(code, self.ctx.author))
                except (discord.Forbidden , discord.HTTPException, discord.InvalidArgument):
                    pass
        ...
        for i in category_data:
            try:
                ...
                await category_channel.edit(
                    ...
                    reason  = self.reason(code, self.ctx.author),
                    overwrites  = await self.return_role_overwrites(category_data[i]['role_overwrites']), 
                ...
                try:
                    await created_category.edit(nsfw = category_data[i]['nsfw'], reason = self.reason(code, self.ctx.author))
                except (discord.Forbidden , discord.HTTPException, discord.InvalidArgument):
                    pass
        ...
        for i in text_data:
            try:
                text_channel = await commands.TextChannelConverter().convert(self.ctx, i)  
                await text_channel.edit(
                    position = text_data[i]["position"],
                    nsfw = text_data[i]["nsfw"],
                    reason  = self.reason(code, self.ctx.author),
                    overwrites  = await self.return_role_overwrites(text_data[i]['role_overwrites']),
                    ...
                    category = await self.return_category_channel(text_data[i]["category"])
                )
            except (commands.ChannelNotFound, commands.BadArgument, commands.CommandError):
                ...
                try:
                    await created_text.edit(default_auto_archive_duration = text_data[i]["default_auto_archive_duration"],permissions_synced = text_data[i]["permissions_synced"], reason = self.reason(code, self.ctx.author))
                except (discord.Forbidden , discord.HTTPException, discord.InvalidArgument):
                    pass
        ...
        for i in voice_data:
            try:
                voice_channel = await commands.VoiceChannelConverter().convert(self.ctx, i)  
                await voice_channel.edit(
                    position = voice_data[i]["position"],
                    reason  = self.reason(code, self.ctx.author),
                    overwrites  = await self.return_role_overwrites(voice_data[i]['role_overwrites']),
                    ...
                    category = await self.return_category_channel(voice_data[i]["category"])
                )
            except (commands.ChannelNotFound, commands.BadArgument, commands.CommandError):
                ...
                try:
                    await created_voice.edit(permissions_synced = voice_data[i]["permissions_synced"], reason = self.reason(code, self.ctx.author))
                except (discord.Forbidden , discord.HTTPException, discord.InvalidArgument):
                    pass
        ...
        for i in stage_data:
            try:
                voice_channel = await commands.StageChannelConverter().convert(self.ctx, i)  
                await voice_channel.edit(
                    position = stage_data[i]["position"],
                    reason  = self.reason(code, self.ctx.author),
                    overwrites  = await self.return_role_overwrites(stage_data[i]['role_overwrites']),
                    ...
                    category = await self.return_category_channel(stage_data[i]["category"])
                )
            except (commands.ChannelNotFound, commands.BadArgument, commands.CommandError):
    ...
    async def return_category_channel(self, name: str) -> Optional[discord.CategoryChannel]:
