...
class VoteInfo(commands.Cog):
    def __init__(self, bot):
        ...
        self.bfd_site = (
            f"[Click Here](https://botsfordiscord.com/bot/{self.bot.user.id})")
        self.discordboats_site = (
            f"[Click Here](https://discord.boats/bot/{self.bot.user.id}/)")
        self.void_bots_site = (
            f"[Click Here](https://voidbots.net/bot/{self.bot.user.id}/)")
        self.fateslist_bots_site = "[Click Here](https://fateslist.xyz/minato/)"
        self.bladebot_bots_site = (
            f"[Click Here](https://bladebotlist.xyz/bot/{self.bot.user.id}/)")
        self.discordlistspacesite = (
            f"[Click Here](https://discordlist.space/bot/{self.bot.user.id})")
    ...
    async def hasvoted(self,
        ...
        async with ctx.channel.typing():
            ...
            g = requests.get(
                f"https://bladebotlist.xyz/api/bots/{self.bot.user.id}/votes/{member.id}",
            )

            h = requests.get(
                f"https://api.discordlist.space/v2/bots/{self.bot.user.id}/status/{member.id}",
            ...
            e.add_field(
                ...
                if a_list else f"Not Voted : {self.topgg_site}",
            )
            e.add_field(
                name="**BotsForDiscord**",
                value=f"Voted : {self.bfd_site}"
                if c_list else f"Not Voted : {self.bfd_site}",
            )
            e.add_field(
                name="**DiscordBoats**",
                value=f"Voted : {self.discordboats_site}"
                if d_list else f"Not Voted : {self.discordboats_site}",
            )
            e.add_field(
                name="**Void Bots**",
                value=f"Voted : {self.void_bots_site}"
                if e12 else f"Not Voted : {self.void_bots_site}",
            )
            e.add_field(
                name="**Fates List**",
                value=f"Voted : {self.fateslist_bots_site}"
                if f1 else f"Not Voted : {self.fateslist_bots_site}",
            )
            e.add_field(
                name="**BladeBotList**",
                value=f"Voted : {self.bladebot_bots_site}"
                if g1 else f"Not Voted : {self.bladebot_bots_site}",
            )
            e.add_field(
                name="**DiscordListSpace**",
                value=f"Voted : {self.discordlistspacesite}"
                if g1 else f"Not Voted : {self.discordlistspacesite}",
            )
