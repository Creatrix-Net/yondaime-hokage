...
class Games(commands.Cog):
    ...
    async def connectfour(self, ctx, member:  Union[int, discord.Member]):
        ...
        if member == ctx.author or member.bot:
            await ctx.send(embed=ErrorEmbed(description='*You cannot play this game yourself or with a bot*'))
            return
        await ctx.send(embed=Embed(description='**Here is the link to know about** *Connect Four*: [Click Here](<https://en.wikipedia.org/wiki/Connect_Four#firstHeading>)'))
    ...
    async def hangman(self, ctx):
        '''Play Hangman!'''
        await ctx.send(embed=Embed(description='**Here is the link to know about** *Hangman*: [Click Here](<https://en.wikipedia.org/wiki/Hangman_(game)#Example_game>)'))
