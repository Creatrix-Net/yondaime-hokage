    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.limit_to_one:
            roles_to_remove = [
                ...
                if r != self.role_id
                and interaction.guild.get_role(r)
                and interaction.guild.get_role(r) in interaction.user.roles
            ]
            if roles_to_remove:
                try:
        ...
        try:
            ...
            await interaction.response.send_message(
                "I don't have permissions to manage that role.", ephemeral=True
            )
...
class ReactionRoles(commands.Cog, name="Reaction Roles"):
    ...
    async def cog_load(self):
        ...
        for message_id_str, data in views.items():
            try:
                self.bot.add_view(
                    PersistentRoleView(data=data), message_id=int(message_id_str)
                )
    ...
    async def new(self, ctx: commands.Context):
        ...
        def reaction_check(payload):
            return (
                ...
                and payload.message_id == sent_limit_message.id
            )
            ...
            if content or emb:
                ...
                try:
                    sent_final = await target_channel.send(
                        content=content, embed=emb, view=PersistentRoleView(data=data)
                    )
    ...
    async def delete_reaction_roles(
        self, ctx: commands.Context, reaction_roles_message: commands.MessageConverter
    ):
